...
- cellW ???`(boardW-3*gap)/4=76` ?????? 8px ??? ? ???????????
  ?? `(boardW-2*pad-3*gap)/4=72`?
- ?????sfx ? LVGL ????????? 300ms ? ?????????/????????
  ?????????? SfxId ???loop() ??? sfxPump() ???
- SOON ?? y=100 ???? 120px ?? 2px ? ?? 94?
- SCORE/BEST"??"?????54px ????/????? 2px ???? + ?????
  ?? 54px + ? font_cjk_20 ????
...
| 3 | I2S??/MCLK?? | ????(I2S????) | ????4?? |
| 4 | QSPI 40MHz ????? | ?? 20MHz | ??????? |
| 5 | CO5300 ??? PWM ???? | 150?180?220 | ????4?? |
| 6 | font_ui_20 ?????? | Python ?????? | ???????? |

**??????????????????**

1. **writePixels ??????**?`l & 1` ??? `MSB_16_SET(_buffer16[l-1], p1)` ?? l ?
   chunk ??? `_buffer16[l-1]` ? uint16 ???? buffer ? byte ???1024 ?? chunk ??
   ?????**????**(? 329px ??)?????????????
2. **writeAddrWindow `_currentX/Y/W/H` ????**?????+flush ???????????
   ????????=????????????? CASET/PASET????????
3. **heap_caps DMA ?????????**?????????????audio ???????
   ????????? `_buffer/_2nd_buffer` ? static ???????
4. **LVGL ?? flush ????**?`draw16bitRGBBitmap` ? `out_width < w` ?????????
   writePixels??????? 328 ??????????
5. **??????**?????? mono_watchface 220 ???????? ?? ???????
   ???? 15_ES8311 demo ????? demo ????????????

**?????????**

- ????????SerialPort COM3 115200, DTR ??????
- ??????????? glyph_bitmap+glyph_dsc ? 4bpp ? ASCII art???????
- ??????????????????????/????
...
1. **??????**?? tick ??????????? flush?CS ??????????
   ...
   ????? flush?mono ????/2048 ???????????????????
2. **??????????????**?
   - `dot_games.ino` **rounder_cb**???????? 368px ??? ? ? tick ??
     ????? 1-3 ???? flush????????????
   - **GFX ???** `Arduino_CO5300.cpp writeAddrWindow`????? CASET/PASET
     ????????????????????????? ? ??????? ?
     ??????????
3. **????**??????+???????? i*3/16 + shownTier[16][16]?
   ? tick ?~1.5 ????????-97%??????"?????? tail+1"?????
   LV_MEM_SIZE 48?96KB?????+256 ???? 48K ? OOM?addr2line ????
4. **????????? skill**?`amoled-watchface` ???????????
   ??????/?????/???????????????????
5. 2048 ? 4 ?????????????????????????????????
